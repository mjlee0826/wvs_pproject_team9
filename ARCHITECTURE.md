...
#### 提問（Questions）
- [x] 列表（cursor-based 分頁，依科目篩選）
- [x] 排序：待解決優先，同狀態內依建立時間新到舊
- [x] 新增討論串（選科目、填標題與內文）
- [x] 看單一討論串（標題、內文、作者、科目）
- [x] 標記為已解決 / 未解決（只有發問者、老師、管理員）
- [x] 列表下拉刷新 + 滑到底自動載入
- [x] 匿名發問：使用者可選擇以匿名方式發問，前端隱藏顯示名稱，後端仍保留作者 ID

#### 回答（Answers）
- [x] 在討論串下新增回答
- [x] 老師可標記為官方解答（顯示「⭐ 老師解答」badge）
- [x] 官方解答排在最上方
- [x] 老師對回答按讚 / 取消讚 → 學生加 1 / 減 1 金幣（用 transaction）

#### 留言（Replies）
- [x] 在回答下新增留言

#### 聊天室（Chat Rooms）
- [x] 聊天室列表（顯示最後一則訊息預覽、訊息總數）
- [x] 依「全部 / 未讀 / 已讀」篩選聊天室
- [x] 進入聊天室即時清除未讀數
- [x] 列表下拉刷新

#### 訊息（Messages）
- [x] 載入聊天室訊息（cursor-based 分頁，舊到新）
- [x] 傳送訊息（REST fallback：socket 未連線時走 HTTP）
- [x] 即時接收新訊息（WebSocket / Socket.IO）
- [x] 即時更新聊天室列表的最後一則訊息預覽
- [x] 未讀計數即時累加（其他人傳來才加，自己傳的不算）
- [x] 自動重連（Socket.IO reconnection）
- [x] 離開聊天室時自動斷線並離開房間（`chat:leave`）

#### 個人頁（Profile）
- [x] 自己的個人頁（編輯名字、換頭像、看自己的貼文、看自己的提問、看金幣進度）
- [x] 看別人的個人頁
